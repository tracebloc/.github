...
MUTATIONS = [
    ...
    # THE ANCHOR NAMES THE HEAD ARM. The terminator class is now shared with
    # the sed and read arms added for backend#2967, so the bare class string
    # matches three times -- and an anchor matching more than once mutates an
    # arbitrary one of them and reports "uncaught" about the wrong arm.
    ...
    # The sed arm's TIGHTNESS, not just its presence. The loose form reports on
    # every `sed 's/…q…/'` in the fleet, which is how a gate gets switched off.
    ("the sed arm matches any q in the sed script, not the script token", AWK,
     "sed[[:space:]]+(-[a-zA-Z]+[[:space:]]+)*'?[0-9]*q'?",
     "sed[^|\\001]*q"),
    # `read` must sit DIRECTLY after the bar, or `| while read` -- which reads to
    # EOF and is the opposite of this class -- reads as a hazard.
