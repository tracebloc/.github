...
- **Fix the class, not the instance.** Name the class the bug belongs to and check the whole class before pushing. Two shapes, and a fix aimed at only the first catches half: other **call sites** (grep the symbol you changed) and other **inputs to the same guard** (what else reaches this branch?). If the class cannot be cheaply enumerated, say so in the PR rather than leaving it unsaid.
